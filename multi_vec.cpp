...
public:
    ...
    void createPatternPerms()
    {
        ...
        for (int i = 0; i < num_perms; i++)
        {
            if (i != 0)
            {
                for (int k = j_vals.size() - 1; k >= 0; k--)
                {
                    int i_val = j_vals[k];
                    cout << "FIRST " << ranges[i_val].first << " SECOND " << ranges[i_val].second << endl;
                    if (ranges[i_val].first != ranges[i_val].second)
                    {
                        ranges[i_val].first += 1;
                        break;
                    }
                    else
                    {
                        ranges[i_val].first = patternParts[i_val].min;
                    }
                }
            }
            ...
            for (const string &word : words[i])
            {
                ...
                else
                {
                            for (PatternPerm pattern : patternPerms)
                            {
                                if (pattern.length == i)
                                {
                                    auto patternParts = pattern.patternParts;
